...
func ListenAndServe(server *http.Server, tlsConfigPath string, logger *logrus.Logger) error {
...
// tlsConfigPath, TLS or basic auth could be enabled.
func Serve(l net.Listener, server *http.Server, tlsConfigPath string, logger *logrus.Logger) error {
	if tlsConfigPath == "" {
		logger.Infof("msg", "TLS is disabled.", "http2", false)
	...
	case nil:
		...
		// Valid TLS config.
		logger.Infof("msg", "TLS is enabled.", "http2", c.HTTPConfig.HTTP2)
	case errNoTLSConfig:
...
func Listen(server *http.Server, tlsConfigPath string, logger *logrus.Logger) error {
